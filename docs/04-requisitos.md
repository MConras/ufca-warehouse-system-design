...
| **RF03** | Notificações de status e prazos | O sistema deve enviar notificações aos funcionários sobre o status de suas solicitações de materiais e sobre os prazos de devolução dos recursos emprestados. | Alta |
...
| **RF07** | Registro de movimentação e estado de conservação | O sistema deve permitir que o gestor de recursos registre as entradas (devoluções) e saídas (empréstimos) de materiais, além de informar o estado de conservação ao receber uma devolução, mantendo atualizadas a disponibilidade e a integridade do estoque. | Alta |
...
| **RF09** | Geração de relatórios de utilização dos recursos | O sistema deve permitir que o gestor de recursos gere relatórios relacionados à utilização dos recursos, incluindo informações como os itens mais emprestados e o tempo médio de utilização. | Baixa |
...
| **RF11** | Cancelamento de solicitação | O sistema deve permitir que o funcionário cancele uma solicitação de material pendente, evitando a separação desnecessária de itens. | Média |
| **RF12** | Alerta de estoque baixo | O sistema deve permitir que o gestor do almoxarifado visualize quais materiais estão com estoque baixo para auxiliar na reposição preventiva. | Alta |
...
| **RNF05** | Disponibilidade | O sistema deve permanecer disponível durante o horário de funcionamento do almoxarifado, possibilitando a realização das operações necessárias para o gerenciamento dos recursos. | O sistema deve garantir disponibilidade contínua durante o horário de atendimento do almoxarifado, sem quedas por falhas internas de aplicação. |